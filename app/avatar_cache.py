...
def sync_live_log_avatars(mapping: dict[str, str]) -> int:
    ...
    for user_id, avatar_id in mapping.items():
        ...
        previous = (
            str(existing.get('avatar_id') or '').strip()
...
def set_cached_avatar_id(user_id: str, avatar_id: str, *, source: str) -> None:
    ...
    if previous_id == avatar_id:
        return
    if (
        source not in _LOG_SOURCES
        and previous_id
        and previous_id != avatar_id
        and previous_source in _LOG_SOURCES
        and source not in ('api', 'instance')
    ):
        return
    cache[user_id] = {
        'avatar_id': avatar_id,
...
def merge_user_avatar_map(mapping: dict[str, str], *, source: str) -> None:
    """Fill cache from historical scans; never overwrite a different live log entry."""
    if not mapping:
        return
    cache = _load()
    changed = False
