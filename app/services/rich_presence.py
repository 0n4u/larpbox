...
class RichPresenceManager(QObject):
    ...
    def update_instance(self, info: InstanceInfo | None) -> None:
        if info is None or not info.world_name:
            self.clear()
            return
        instance_key = f'{info.world_id}:{info.instance_id}'
        key = f'{instance_key}:{info.player_count}'
        if key == self._last_key:
            return
                                                                        
        if not self._last_key.startswith(instance_key):
        ...
        if self._discord_enabled() and self._ensure_discord():
            payload = self._build_payload(info, world, details, state)
            try:
                self._discord.update(**payload)
            except Exception:
                logger.debug('Discord presence update failed', exc_info=True)
                self._discord = None

    def _build_payload(self, info: InstanceInfo, world: str, details: str, state: str) -> dict:
        payload: dict = {
            'details': world, 'state': state,
            'large_image': 'vrchat', 'large_text': world, 'small_text': details,
        }
        if self._since_ts:
            payload['start'] = int(self._since_ts)
        if info.max_players and info.player_count:
            payload['party_size'] = [int(info.player_count), int(info.max_players)]
        if info.world_id.startswith('wrld_'):
            payload['buttons'] = [{'label': 'View World', 'url': f'https://vrchat.com/home/world/{info.world_id}'}]
        return payload
