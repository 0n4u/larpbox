...
from ..config import clear_auth_session
...
class SessionManager(QObject):
    ...
    def _run_health_check(self) -> None:
        ...
        self._verify_worker.finished.connect(self._on_health_check_finished)
        self._verify_worker.start()

    def _on_health_check_finished(self, result: object) -> None:
        if self._relogin_active:
            return
        if isinstance(result, VRChatSession):
            if result.auth_token != self._session.auth_token or result.two_factor_token != self._session.two_factor_token:
