...
class _AvatarLogRefreshWorker(QThread):
    finished = pyqtSignal(int)

    def run(self) -> None:
        try:
            from ..avatar_cache import refresh_from_logs
            self.finished.emit(refresh_from_logs())
        except Exception:
            logger.debug('Background avatar log refresh failed', exc_info=True)
...
class SessionManager(QObject):
    ...
    def __init__(self) -> None:
        ...
        self._log_refresh_worker: _AvatarLogRefreshWorker | None = None
    ...
    def mark_login_success(self) -> None:
        if self._session is None:
            return
        self._remember_account(self._session)
        from ..vrchat.core import clear_session_caches
        ...
        def _refresh_panels() -> None:
            ...
            if hasattr(window, 'account_info'):
                QTimer.singleShot(0, window.account_info.refresh)
            if hasattr(window, 'friends_list'):
                QTimer.singleShot(400, lambda: window.friends_list.refresh(full=False))
            if hasattr(window, 'player_list'):
                QTimer.singleShot(900, window.player_list.refresh)
    ...
    def _schedule_log_refresh(self) -> None:
        if self._log_refresh_worker is not None and self._log_refresh_worker.isRunning():
            return
        self._log_refresh_worker = _AvatarLogRefreshWorker()
        self._log_refresh_worker.finished.connect(self._on_log_refresh_done)
        self._log_refresh_worker.start()

    def _on_log_refresh_done(self, count: int) -> None:
        if count > 0:
            logger.debug('Avatar cache refreshed from logs (+%d entries)', count)

    def _remember_account(self, session: VRChatSession) -> None:
        """Record the signed-in account (metadata always; secrets only if remembered)."""
        if not session.user_id:
            return
        try:
            from ..store import meta_repo
            meta_repo.upsert_account(user_id=session.user_id, username=session.username, display_name=session.display_name)
        except Exception:
            logger.debug('Could not record account metadata', exc_info=True)
        if not get_bool(load_config().get('remember_login')):
            return
        try:
            from ..secret_store import save_account_secrets
            save_account_secrets(session.user_id, {
                'auth_token': session.auth_token,
                'two_factor_token': session.two_factor_token or '',
                'auth_username': session.username,
                'auth_display_name': session.display_name,
                'auth_user_id': session.user_id,
            })
        except Exception:
            logger.debug('Could not store per-account credentials', exc_info=True)

    def apply_switched_session(self, session: VRChatSession) -> None:
        """Apply a session selected via the multi-account switcher to the whole app."""
        from ..config import save_auth_session
        self.set_session(session)
        if get_bool(load_config().get('remember_login')):
            save_auth_session(
                auth_token=session.auth_token, two_factor_token=session.two_factor_token,
                username=session.username, display_name=session.display_name,
                user_id=session.user_id, remember_login=True,
            )
        window = self._main_window
        if window is not None and hasattr(window, 'apply_session'):
            window.apply_session(session)
