...
from .auth_errors import is_auth_failure, is_rate_limit_error
...
def report_api_error(context: str, exc: Exception) -> None:
    ...
    if is_rate_limit_error(exc):
        ErrorBus.instance().warning(f'{context}: VRChat rate limit — wait a moment.')
        return
    message = str(exc).strip() or type(exc).__name__
    ErrorBus.instance().warning(f'{context}: {message[:160]}')


def report_worker_error(context: str, exc: object) -> None:
    if isinstance(exc, BaseException):
        report_api_error(context, exc)
        return
    text = str(exc).strip() or 'Request failed.'
    ErrorBus.instance().warning(f'{context}: {text[:160]}')
