...
from .http_proxy import proxy_status_label, urlopen as proxy_urlopen
...
def probe_provider_search(provider_id: str, label: str, search_fn: Callable[..., list[AvatarResult]]) -> ProviderProbeResult:
    try:
...
def _probe_api_url(url: str) -> tuple[bool, str]:
    ...
    try:
        with proxy_urlopen(req, timeout=15) as resp:
            body = resp.read(400)
