...
from ..theme import COLOR_TEXT, COLOR_TEXT_BRIGHT, COLOR_TEXT_SUBTLE, dark_theme
...
class NotificationOverlay(QWidget):
    ...
    def __init__(self):
        ...
        self._vrchat_cache: tuple[object, float] | None = None
    ...
    def _cached_vrchat_window(self, *, max_age_sec: float=1.5):
        now = time.monotonic()
        if self._vrchat_cache is not None:
            info, cached_at = self._vrchat_cache
            if now - cached_at < max_age_sec:
                return info
        info = find_vrchat_window()
        self._vrchat_cache = (info, now)
        return info
    ...
    def _vrchat_available(self) -> bool:
        return self._cached_vrchat_window() is not None
    ...
    def _apply_window_geometry(self) -> None:
        info = self._cached_vrchat_window()
        if info is None:
            return
        _, rect = info
