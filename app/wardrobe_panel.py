...
from .ui_scroll import ScrollDebouncer
...
class WardrobePanel(QWidget):
    ...
    def __init__(self, session: VRChatSession | None=None, parent: QWidget | None=None):
        ...
        self._scroll_populate_debouncer = ScrollDebouncer(self, self._on_scroll_populate, delay_ms=80)
        self._populate_inflight = False
        self._build_ui()
    ...
    def _on_scroll(self, _value: int) -> None:
        self._scroll_populate_debouncer.schedule()

    def _on_scroll_populate(self) -> None:
        bar = self.scroll.verticalScrollBar()
        if bar.maximum() - bar.value() > _SCROLL_LOAD_MARGIN_PX:
            return
        if self._populate_index >= len(self._filtered_avatars):
            return
        if self._populate_inflight:
            return
        self._populate_inflight = True
        self._populate_next_batch()
