...
_STATUS_STYLES: dict[str, tuple[str, str]] = {
    'active': ('Online', '#4cd964'),
    'join me': ('Join Me', '#4ea3ff'),
    'ask me': ('Ask Me', '#e8a040'),
    'busy': ('Do Not Disturb', '#e07070'),
    'offline': ('Offline', '#6a6a6a'),
    'invisible': ('Invisible', '#c084fc'),
    'private': ('Private', '#9aa0a6'),
}
...
def status_dot_style(color: str, *, size: int = 8) -> str:
    radius = max(2, size // 2)
    return (
        f'background-color: {color}; border: 1px solid rgba(0, 0, 0, 0.45); '
        f'border-radius: {radius}px; min-width: {size}px; max-width: {size}px; '
...
def user_status_from_user(user: Any, *, log_fallback: str | None = None) -> UserStatusInfo:
    ...
    in_world = bool(
        location
        and not location.casefold().startswith('offline')
    )
    ...
    if location and location.casefold().startswith('private'):
        return UserStatusInfo(
            key='private',
            label='Private',
            color=_STATUS_STYLES['private'][1],
            in_world=True,
            location=location,
        )
    ...
    if in_world and status_text == 'offline':
        return UserStatusInfo(
            key='invisible',
            label='Invisible',
            color=_STATUS_STYLES['invisible'][1],
            in_world=True,
            location=location,
        )
    ...
    if status_text in _STATUS_STYLES:
        ...
        return UserStatusInfo(
            key=status_text.replace(' ', '_'),
            label=label,
            color=color,
            in_world=in_world,
            location=location if in_world else None,
        )
