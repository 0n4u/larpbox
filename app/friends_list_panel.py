...
from PyQt6.QtWidgets import QApplication, QFrame, QGridLayout, QHBoxLayout, QLabel, QScrollArea, QSizePolicy, QVBoxLayout, QWidget
...
from .vrchat_api import FriendEntry, force_clone_player_avatar, get_friends_list, join_player_instance, moderate_player, unmoderate_player, unfriend_user, user_profile_url
...
class FriendsListPanel(QWidget):
    ...
    def __init__(self, session: VRChatSession | None=None, parent: QWidget | None=None):
        ...
        self._poll_timer.timeout.connect(lambda: self.refresh(full=True))
        self._build_ui()
        self._show_hint(_HINT)
        if self.session is not None:
            RemoteImageLabel.set_session(self.session)
            self._poll_timer.start()
            QTimer.singleShot(800, lambda: self.refresh(full=False))
    ...
    def _on_card_action(self, action: str, user_id: str, detail: str) -> None:
        ...
        if action == 'force_clone':
            ...
            self._action_worker = ApiActionWorker(lambda: force_clone_player_avatar(self.session, user_id, display_name=detail or None), 'Avatar selected — switch applies in VRChat.', timeout_sec=_FORCE_CLONE_TIMEOUT_SEC)
        ...
        if action in mod_map:
            ...
            self._action_worker = ApiActionWorker(lambda: fn(self.session, user_id, mod_type))
            self._action_worker.finished_ok.connect(self._on_action_ok)
            self._action_worker.finished_error.connect(self._on_action_error)
            self._action_worker.start()
            return
        if action == 'unfriend':
            self._action_worker = ApiActionWorker(lambda: unfriend_user(self.session, user_id))
            self._action_worker.finished_ok.connect(self._on_action_ok)
            self._action_worker.finished_error.connect(self._on_action_error)
            self._action_worker.start()
    ...
    def _on_action_error(self, message: str) -> None:
        if SessionManager.instance().try_handle_auth_failure(message):
            self._set_status_message('Session expired — sign in again', error=True)
            return
    ...
    def _emit_friend_notifications(self, friends: list[FriendEntry]) -> None:
        ...
        for friend in friends:
            ...
            if friend.can_join and prev != 'join_me':
