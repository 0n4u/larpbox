...
from .theme import FRIEND_THUMB_STYLE, TOOLBAR_BUTTON, COLOR_ERROR, dark_theme, themed_menu
...
from .config import get_bool, load_config, load_config_cached, save_config
...
from .ui_scroll import ScrollDebouncer, visible_widgets_in_host
...
_POLL_INTERVAL_MS = 30000
_FULL_POLL_INTERVAL_MS = 90000

class FriendsWorker(QThread):
    finished_ok = pyqtSignal(object)
    finished_error = pyqtSignal(object)

    def __init__(self, session: VRChatSession, *, include_offline: bool=True, enrich_worlds: bool=True, max_avatar_resolves: int=16):
        super().__init__()
        self.session = session
        self.include_offline = include_offline
        self.enrich_worlds = enrich_worlds
        self.max_avatar_resolves = max_avatar_resolves
    ...
    def run(self) -> None:
        try:
            friends = get_friends_list(
                ...
                enrich_worlds=self.enrich_worlds,
                max_avatar_resolves=self.max_avatar_resolves,
            )
...
class FriendCard(QFrame):
    ...
    def __init__(self, friend: FriendEntry, parent: QWidget | None=None, *, session: VRChatSession | None=None):
        ...
        self.thumb.setStyleSheet(FRIEND_THUMB_STYLE)
    ...
    def _show_context_menu(self, pos) -> None:
        ...
        try:
            ...
            try:
                ...
                fav_on = favorites_repo.is_favorite(kind='friend', target_id=self.friend.user_id)
            except Exception:
                fav_on = False
            local_fav = menu.addAction('Remove local favorite' if fav_on else 'Add local favorite')
            local_fav.setToolTip('Local-only favorite (no VRChat limit)')
            local_fav.triggered.connect(lambda: self.action_requested.emit('local_favorite', self.friend.user_id, self.friend.display_name))
...
class FriendsListPanel(QWidget):
    ...
    def __init__(self, session: VRChatSession | None=None, parent: QWidget | None=None):
        ...
        self._filter_id = str(load_config_cached().get('friend_filter', 'all'))
        self._friends_ever_loaded = False
        self._animate_populate = False
        self._pending_refresh_full = False
        self._refresh_debouncer = RefreshDebouncer(self, self._run_refresh, delay_ms=300)
        self._thumb_push_timer = QTimer(self)
        self._thumb_push_timer.setSingleShot(True)
        self._thumb_push_timer.setInterval(800)
        self._thumb_push_timer.timeout.connect(self._push_thumbnails_to_player_list)
        self._friend_image_debouncer = ScrollDebouncer(self, self._load_visible_friend_images, delay_ms=60)
    ...
    def _build_ui(self) -> None:
        ...
        self.scroll.verticalScrollBar().valueChanged.connect(lambda _v: self._friend_image_debouncer.schedule())
    ...
    def _load_visible_friend_images(self) -> None:
        cards = visible_widgets_in_host(self.scroll, self.grid_host, FriendCard, margin_px=FRIEND_CARD_HEIGHT)
        loaded = 0
        for card in cards:
            if loaded >= 12:
                break
    ...
    def _set_status_message(self, text: str, *, error: bool=False, pin_sec: float=0) -> None:
        ...
        if pin_sec > 0:
            ...
            self._status_pinned_until = 0.0
        if error:
            self.status_label.setStyleSheet(f'color: {COLOR_ERROR}; font-size: 8pt;')
        else:
    ...
    def _on_card_action(self, action: str, user_id: str, detail: str) -> None:
        ...
        if action == 'copy_avatar_id':
            ...
            if not avatar_id and self.session is not None:
                from .vrchat_api import resolve_known_avatar_id
                avatar_id = resolve_known_avatar_id(user_id, detail or None, session=self.session)
            if avatar_id:
        ...
        if action == 'open_profile':
            ...
            return
        if action == 'local_favorite':
            try:
                from .store import favorites_repo
                now_fav = favorites_repo.toggle_favorite(kind='friend', target_id=user_id, name=detail or user_id)
                self._set_status_message('Added to local favorites' if now_fav else 'Removed from local favorites', pin_sec=2.5)
            except Exception:
                self._set_status_message('Could not update favorite', error=True, pin_sec=3.0)
            return
        ...
        if action == 'force_clone':
            ...
            self._start_action_worker(ApiActionWorker(lambda: force_clone_player_avatar(self.session, user_id, display_name=detail or None, avatar_id=friend.avatar_id if friend else None, status=status), 'Avatar selected — switch applies in VRChat.', timeout_sec=_FORCE_CLONE_TIMEOUT_SEC, context='force clone'))
            return
        if action == 'join':
            if not detail:
    ...
    def _set_filter(self, filter_id: str) -> None:
        ...
        if self._all_friends:
            self._render_friends(self._all_friends, animate=True)
    ...
    def _render_friends(self, friends: list[FriendEntry], *, animate: bool=False, notify: bool=False) -> None:
        if notify and get_bool(load_config_cached().get('enable_notifications', True)):
            self._emit_friend_notifications(friends)
        display_friends, hidden_offline = self._friends_for_display(friends)
        self._stop_populate()
        self._clear_grid()
        self._animate_populate = animate
    ...
    def refresh(self, *, full: bool=False) -> None:
        ...
        if self._worker and self._worker.isRunning():
            self._pending_refresh_full = self._pending_refresh_full or full
            return
        self._pending_refresh_full = full
        self._run_refresh()

    def _run_refresh(self) -> None:
        if SessionManager.instance().is_relogin_active() or self.session is None:
            return
        full = self._pending_refresh_full
        self._pending_refresh_full = False
        self._fetch_generation += 1
        ...
        self._worker = FriendsWorker(
            self.session,
            include_offline=full,
            enrich_worlds=full,
            max_avatar_resolves=12 if full else 0,
        )
    ...
    def _on_error(self, message: object, generation: int) -> None:
        ...
        if SessionManager.instance().try_handle_auth_failure(message):
            ...
            return
        report_worker_error('Friends list', message)
    ...
    def _on_loaded(self, friends_obj: object, generation: int) -> None:
        ...
        first_load = not self._friends_ever_loaded
        self._friends_ever_loaded = True
        self._render_friends(friends, animate=first_load and should_animate_lists(), notify=True)
        flash_widget(self.status_label)
        if self._pending_refresh_full:
    ...
    def _populate_next_batch(self) -> None:
        ...
        end = min(start + list_populate_batch(normal=_POPULATE_BATCH), len(friends))
        ...
        if self._animate_populate and new_cards:
            animate_list_items(new_cards, pop=True, duration=200, step_ms=20)
        self._friend_image_debouncer.schedule()
        self._populate_index = end
        if end < len(friends):
            delay = list_populate_delay_ms(animated=self._animate_populate)
