...
_DEFAULT_MIN_INTERVAL = 0.45
_DEFAULT_MAX_PER_MINUTE = 36
_DEFAULT_COOLDOWN_SEC = 12.0
_MAX_RETRIES = 3
        ...
        while True:
            with self._lock:
                ...
                if now < self._cooldown_until:
                    ...
                    self._request_times = [
                        stamp for stamp in self._request_times if now - stamp < 60.0
                    ]
    ...
    def note_rate_limited(self, *, retry_after: float = _DEFAULT_COOLDOWN_SEC) -> None:
        with self._lock:
            self._cooldown_until = max(
                ...
                time.monotonic() + max(retry_after, 1.0),
            )
