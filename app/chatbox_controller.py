...
from app.config import load_config, message_interval_ms, parse_idle_char, VRCHAT_CHATBOX_MIN_INTERVAL_MS
from app.logging_setup import get_logger, is_debug_mode, truncate_for_log
from app.osc_client import OscChatClient
...
class OSCHandler(QObject):
    ...
    def _send_generic_osc(self, address: str, arguments: list[Any]) -> None:
        ...
        if self.use_secondary_osc and self.secondary_client:
            self.secondary_client.send_message(address, list(arguments))
    ...
    def _send_typing_message(self, typing: bool) -> None:
        try:
            ...
            if self.use_secondary_osc and self.secondary_client:
                self.secondary_client.send_message('/chatbox/typing', typing)
    ...
    def _send_input_message(self, message: str, fx: bool) -> None:
        ...
        try:
            ...
            self.client.send_message('/chatbox/input', [formatted, True, fx])
        if self.use_secondary_osc and self.secondary_client:
            self.secondary_client.send_message('/chatbox/input', [formatted, True, fx])
    ...
    def _send_raw_message(self, message: str) -> None:
        ...
        if self.use_secondary_osc and self.secondary_client:
            self.secondary_client.send_message('/chatbox/input', [message, True, False])
    ...
    def load_settings(self):
        ...
        if CONFIG_PATH.exists():
            ...
            self.use_secondary_osc = config.get('use_secondary_osc', False)
            self.secondary_osc_ip = config.get('secondary_osc_ip', '127.0.0.1')
            self.secondary_osc_port = config.get('secondary_osc_port', 9001)
            ...
            if port != 9000:
                logger.warning(
                    'OSC port is %s — VRChat receives on 9000 by default (Action Menu → OSC → Enabled)',
                    port,
                )
            self.client = udp_client.SimpleUDPClient(ip, port)
            try:
        ...
        if self.use_secondary_osc:
            ...
            sec_port = getattr(self, 'secondary_osc_port', 9001)
