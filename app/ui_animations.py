...
def fade_in_widget(widget: QWidget, *, duration: int=240, delay_ms: int=0, start_opacity: float=0.0, end_opacity: float=1.0, easing: QEasingCurve.Type=_EASE_IN) -> QPropertyAnimation | None:
    if widget is None:
        return None
...
def fade_out_widget(widget: QWidget, *, duration: int=180, delay_ms: int=0, end_opacity: float=0.0, easing: QEasingCurve.Type=_EASE_OUT, on_finished: Callable[[], None] | None=None, hide_after: bool=True) -> QPropertyAnimation | None:
...
def pop_in_widget(widget: QWidget, *, duration: int=260, delay_ms: int=0, start_opacity: float=0.0, end_opacity: float=1.0) -> QPropertyAnimation | None:
...
def stagger_fade_in(widgets: list[QWidget], *, duration: int=200, step_ms: int=28, easing: QEasingCurve.Type=_EASE_SMOOTH) -> None:
    for index, widget in enumerate(widgets):
        if widget is None:
            continue
        fade_in_widget(widget, duration=duration, delay_ms=index * step_ms, easing=easing)

def stagger_pop_in(widgets: list[QWidget], *, duration: int=220, step_ms: int=24) -> None:
    for index, widget in enumerate(widgets):
        if widget is None:
            continue
        pop_in_widget(widget, duration=duration, delay_ms=index * step_ms)

def reveal_content(hide: QWidget | None, show: QWidget, *, duration: int=260, pop: bool=True) -> None:
    ...
    if hide is not None and hide.isVisible():
        fade_out_widget(hide, duration=max(120, duration // 2), on_finished=_show, hide_after=True)
        return
    _show()
