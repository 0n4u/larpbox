...
from .vrchat_log_players import (
    LogPlayer,
    log_players_for_current_room,
    lookup_player_avatar_info,
    lookup_player_avatar_info_historical,
    parse_avatar_ids_from_log,
    current_room_user_ids,
)
...
_FILE_ID_RE = re.compile(r'(file_[a-f0-9-]+)', re.IGNORECASE)
_THUMBNAIL_IMAGE_RE = re.compile(
...
_VRCHAT_HIDDEN_AVATAR_FILE_IDS = frozenset({
    'file_0e8c4e32-7444-44ea-ade4-313c010d4bae',
})
...
_TRUST_RANKS: tuple[tuple[str, str, str, str], ...] = (
    ('system_trust_legend', 'Legend', 'LEG', '#e8a040'),
    ('system_trust_veteran', 'Veteran', 'VET', '#c084fc'),
    ('system_trust_trusted', 'Trusted', 'TRU', '#4ea3ff'),
    ('system_trust_known', 'Known', 'KNW', '#7db87d'),
    ('system_trust_basic', 'User', 'USR', '#9aa0a6'),
)
...
def normalize_thumbnail_url(url: str) -> str:
    """Return a loadable VRChat image URL, or empty if the URL cannot be rendered."""
    cleaned = (url or '').strip()
    if not cleaned or 'cloudfront.net' in cleaned:
        return ''
    if '/api/1/file/' in cleaned and '/api/1/image/' not in cleaned:
        return ''
    match = _THUMBNAIL_IMAGE_RE.search(cleaned)
    if match:
        file_id, size = match.groups()
        return f'https://api.vrchat.cloud/api/1/image/{file_id}/1/{size}'
    if 'api.vrchat.cloud' in cleaned or cleaned.startswith('http'):
        return cleaned
    return ''
    ...
    for friend in friends:
        ...
        index.setdefault(key, []).append(
            InstancePlayer(
                ...
                thumbnail_url=normalize_thumbnail_url(friend.thumbnail_url),
                is_friend=True,
                trust=friend.trust,
            )
        )
...
def make_api_client(session: VRChatSession) -> vrchatapi.ApiClient:
    ...
    api_client.call_api = rate_limited_call_api  # type: ignore[method-assign]
    return api_client
...
def _avatar_from_api(avatar: Any) -> AvatarResult:
    ...
    return AvatarResult(
        ...
        name=str(getattr(avatar, 'name', '') or 'Unknown'),
        description=str(getattr(avatar, 'description', '') or '').strip(),
        author_name=str(getattr(avatar, 'author_name', '') or ''),
        image_url=str(getattr(avatar, 'thumbnail_image_url', '') or getattr(avatar, 'image_url', '') or ''),
...
def _avatar_from_avtrdb(item: dict[str, Any]) -> AvatarResult:
    ...
    return AvatarResult(
        ...
        image_url=str(item.get('imageUrl') or item.get('thumbnailImageUrl') or ''),
        performance=str(pc_rating) if pc_rating else None,
...
def search_avatars_requi(query: str, *, limit: int = 40) -> list[AvatarResult]:
    ...
    if len(query) < 3:
        return []
    param_variants = (
        urllib.parse.urlencode({'search': query, 'n': str(limit)}),
        urllib.parse.urlencode({'searchTerm': query, 'limit': str(limit)}),
    )
    last_error: Exception | None = None
...
def search_avatars_combined(query: str, *, limit: int = 40) -> list[AvatarResult]:
    query = query.strip()
    if len(query) < 2:
        return []
    merged: dict[str, AvatarResult] = {}
    sources = (
        lambda q, n: search_avatars_avtrdb(q, limit=n),
        lambda q, n: search_avatars_vrcx_endpoint(
            'https://api.avatarrecovery.com/Avatar/vrcx', q, limit=n
        ),
        lambda q, n: search_avatars_requi(q, limit=n),
    )
    per_source = max(20, limit // len(sources) + 1)
    for search in sources:
        try:
            for result in search(query, per_source):
...
def _instance_player_from_limited(
    user: Any,
    *,
    is_friend: bool = False,
    avatar_id: str | None = None,
    ...
    return InstancePlayer(
        user_id=user_id,
        display_name=str(getattr(user, 'display_name', '') or 'Unknown'),
        thumbnail_url=_profile_image_url(user),
        is_friend=is_friend or bool(getattr(user, 'is_friend', False)),
        trust=trust,
        avatar_id=avatar_id,
    )


def _instance_player_from_log(
    ...
    *,
    ...
    return InstancePlayer(
        user_id=log_player.user_id,
        display_name=log_player.display_name,
        thumbnail_url='',
        is_friend=is_friend,
        trust=None,
        avatar_id=avatar_id,
...
def get_current_instance_log_only(
    session: VRChatSession | None = None,
    *,
    enrich: bool = True,
    max_enrich_requests: int = 6,
) -> InstanceInfo | None:
    ...
    players = [
        _instance_player_from_log(
            player,
            avatar_id=avatar_ids.get(player.user_id),
        )
    ...
    if session is not None and enrich:
        try:
            with make_api_client(session) as api_client:
                players = _enrich_players_from_api(
                    api_client,
                    players,
                    session=session,
                    max_requests=max_enrich_requests,
                )
        except Exception:
...
def _enrich_log_player(
    ...
    *,
    ...
    try:
        ...
        player = _instance_player_from_limited(
            user,
            is_friend=log_player.user_id in friend_ids or bool(getattr(user, 'is_friend', False)),
        )
        if player.display_name == 'Unknown' and log_player.display_name:
            return InstancePlayer(
                ...
                trust=player.trust,
                avatar_id=resolve_player_avatar_id(player.user_id, log_player.display_name) or player.avatar_id,
            )
        return player
    except Exception:
        ...
        return InstancePlayer(
            ...
            display_name=log_player.display_name,
            thumbnail_url='',
            is_friend=log_player.user_id in friend_ids,
            trust=None,
            avatar_id=resolve_player_avatar_id(log_player.user_id, log_player.display_name),
        )
    ...
    for group in groups:
        for player in group:
            ...
            if existing is None:
                ...
                continue
            merged[player.user_id] = InstancePlayer(
                user_id=player.user_id,
                display_name=player.display_name or existing.display_name,
                thumbnail_url=player.thumbnail_url or existing.thumbnail_url,
                is_friend=existing.is_friend or player.is_friend,
                trust=player.trust or existing.trust,
                avatar_id=player.avatar_id or existing.avatar_id,
            )
...
def _enrich_players_from_api(
    ...
    *,
    ...
    ordered = sorted(
        players,
        key=lambda player: (
            player.trust is not None and bool(player.thumbnail_url),
    ...
    for player in ordered:
        ...
        enriched.append(
            InstancePlayer(
                user_id=player.user_id,
                display_name=player.display_name,
                thumbnail_url=thumb or player.thumbnail_url,
                is_friend=player.is_friend or bool(getattr(user, 'is_friend', False)),
                trust=trust,
                avatar_id=player.avatar_id,
            )
        )
...
def _enrich_missing_thumbnails(
    api_client: vrchatapi.ApiClient,
    players: list[InstancePlayer],
    *,
    max_requests: int = 16,
...
def _profile_image_url(user: Any) -> str:
    for attr in (
        ...
        value = normalize_thumbnail_url(str(getattr(user, attr, '') or ''))
        if value:
            return value
    return ''
    ...
    for badge in getattr(user, 'badges', None) or []:
        ...
        badges.append(
            UserBadge(
                ...
                name=str(getattr(badge, 'badge_name', '') or 'Badge'),
                image_url=str(getattr(badge, 'badge_image_url', '') or ''),
                showcased=bool(getattr(badge, 'showcased', False)),
            )
        )
...
def _friend_from_user(user: Any) -> FriendEntry:
    ...
    return FriendEntry(
        ...
        trust=trust_rank_from_tags(tags),
        status=status,
        badges=_badges_from_user(user)[:2],
    )
...
def _instance_summary(
    ...
    player_count = int(
        getattr(instance, 'user_count', 0) or getattr(instance, 'n_users', 0) or 0
    )
    return world_name, player_count or None


def _cached_instance_summary(
    api_client: vrchatapi.ApiClient,
    location: str,
) -> tuple[str | None, int | None]:
...
_FRIEND_STATUS_SORT_ORDER: dict[str, int] = {
    'join_me': 0,
    'active': 1,
    'invisible': 1,
    'private': 1,
...
def _fetch_friend_users(api: friends_api.FriendsApi) -> list[Any]:
    users: list[Any] = []
    seen: set[str] = set()
...
def get_friends_list(session: VRChatSession) -> list[FriendEntry]:
    friends: list[FriendEntry] = []
    try:
        with make_api_client(session) as api_client:
            api = friends_api.FriendsApi(api_client)
            for friend in _fetch_friend_users(api):
                friends.append(_friend_from_user(friend))
            friends = _enrich_joinable_friends(api_client, friends)
    except Exception:
    ...
    for friend in friends:
        ...
        if thumb != friend.thumbnail_url:
            friend = FriendEntry(
                ...
                player_count=friend.player_count,
            )
    ...
    try:
        with make_api_client(session) as api_client:
            ...
            try:
                ...
                if getattr(exc, 'status', None) == 401:
                    logger.debug('VRChat auth expired or missing for profile lookup')
                    return None
                raise
            ...
            return CurrentUserProfile(
                user_id=str(getattr(me, 'id', '') or session.user_id),
                display_name=str(getattr(me, 'display_name', '') or session.display_name or 'Unknown'),
                bio=description,
                status_description=status_description,
                image_url=_profile_image_url(me),
                trust=trust_rank_from_tags(tags),
                badges=_badges_from_user(me),
                status=status,
                world_name=world_name,
...
def get_current_instance(
    session: VRChatSession,
    *,
    max_enrich_requests: int = 8,
    friend_players_cache_only: bool = True,
) -> InstanceInfo | None:
    log_players, log_location = log_players_for_current_room()
    avatar_ids = parse_avatar_ids_from_log()
    sync_live_log_avatars(avatar_ids)
    ...
    try:
        with make_api_client(session) as api_client:
            ...
            if instance is not None:
                world = getattr(instance, 'world', None)
                world_name = str(getattr(world, 'name', '') or 'Unknown World') if world else 'Unknown World'
                api_players = [
                    _instance_player_from_limited(
                        user,
                        is_friend=bool(getattr(user, 'is_friend', False)),
                        avatar_id=avatar_ids.get(str(getattr(user, 'id', '') or '')),
                    )
                    for user in (getattr(instance, 'users', None) or [])
                ]
                owner_id = str(getattr(instance, 'owner_id', '') or '')
                api_count = int(
                    getattr(instance, 'user_count', 0) or getattr(instance, 'n_users', 0) or len(api_players)
                )
                if not location:
                    location = str(getattr(instance, 'location', '') or '') or log_location

            friend_players = _players_from_friends(
                api_client,
                location or '',
                cache_only=friend_players_cache_only,
            )
            ...
            log_enriched = [
                _instance_player_from_log(
                    player,
                    is_friend=player.user_id in api_friend_ids,
                    avatar_id=avatar_ids.get(player.user_id),
                )
                for player in log_players
            ]
            me_player = _instance_player_from_limited(
                me,
                avatar_id=avatar_ids.get(str(getattr(me, 'id', '') or session.user_id)),
            )
            ...
            return InstanceInfo(
                ...
                players=players,
                owner_id=owner_id,
                can_close_instance=owner_id == session.user_id,
            )
    except Exception:
...
def _avatar_name_query_variants(name: str) -> list[str]:
    ...
    if ' - ' in name:
        add(name.split(' - ', 1)[0])
    by_parts = re.split(r'\s+by\s+', name, maxsplit=1, flags=re.IGNORECASE)
    if len(by_parts) > 1:
        add(by_parts[0])
    return variants


def _pick_best_avatar_match(
    ...
    *,
    ...
    for query in _avatar_name_query_variants(avatar_name):
        ...
        for result in search_avatars_combined(search_query, limit=40):
            if result.id and result.id not in merged:
                merged[result.id] = result
        picked = _pick_best_avatar_match(
            list(merged.values()),
            avatar_name,
...
def _lookup_avatar_id_by_thumbnail_match(
    session: VRChatSession,
    file_id: str,
    avatar_name: str,
    *,
        ...
        with make_api_client(session) as api_client:
            ...
            for result in results:
                ...
                for field in ('thumbnail_image_url', 'image_url'):
                    candidate_file = _file_id_from_vrchat_asset_url(
                        str(getattr(avatar, field, '') or '')
                    )
    ...
    return lookup_avatar_id_by_name(
        avatar_name,
        user_id=user_id,
...
def _lookup_avatar_id_by_owner_match(
    avatar_name: str,
    user_id: str,
    *,
    image_file_id: str | None = None,
    ...
    partial = [
        result for result in owned
        if target in result.name.casefold() or result.name.casefold() in target
    ]
    if len(partial) == 1:
        return partial[0].id
...
def _resolve_avatar_from_thumbnail_file(
    session: VRChatSession,
    user_id: str,
    file_id: str,
    *,
    ...
    if resolved:
        return resolved
    return _lookup_avatar_id_by_owner_match(
        avatar_name,
        user_id,
        image_file_id=file_id,
    )
...
def _resolve_avatar_id_from_instance_presence(
    session: VRChatSession,
    user_id: str,
...
_JOIN_FOR_CURRENT_AVATAR = (
    'Join their instance to get their current avatar. '
    'VRChat does not expose their latest avatar unless you are in-world together.'
)


def _resolve_avatar_id_from_user_api(
    session: VRChatSession,
    user_id: str,
    *,
    ...
    thumbnail_url = str(
        getattr(user, 'current_avatar_thumbnail_image_url', '')
        or getattr(user, 'current_avatar_image_url', '')
        or ''
    )
    file_id = _file_id_from_vrchat_asset_url(thumbnail_url)
...
def resolve_player_avatar_id(
    user_id: str,
    display_name: str | None = None,
    *,
    session: VRChatSession | None = None,
    for_force_clone: bool = False,
) -> str | None:
    ...
    if session is not None and not in_current_room:
        api_resolved = _resolve_avatar_id_from_user_api(
            session,
            user_id,
    ...
    if not in_current_room:
        ...
        if hist_info.avatar_name:
            resolved = lookup_avatar_id_by_name(
                hist_info.avatar_name,
                hist_info.author_name or None,
                user_id=user_id,
            )
            if resolved:
                set_cached_avatar_id(user_id, resolved, source='log_name')
