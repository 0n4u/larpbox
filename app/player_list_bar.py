...
class PlayerListBar(QWidget):
    ...
    def _on_player_action(self, action: str, user_id: str, display_name: str) -> None:
        try:
            ...
            if action == 'force_clone':
                ...
                self._start_action_worker(ApiActionWorker(lambda: force_clone_player_avatar(self.session, user_id, display_name=display_name or None, avatar_id=avatar_id, status=player_status), 'Avatar selected — switch applies in VRChat.', timeout_sec=_FORCE_CLONE_TIMEOUT_SEC, context='force clone'))
                return
            if action == 'invite':
    ...
    def _apply_instance_info(self, info: InstanceInfo) -> None:
        ...
        def _show_players() -> None:
            RemoteImageLabel.prefetch([player.thumbnail_url for player in info.players if player.thumbnail_url])
