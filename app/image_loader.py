...
class RemoteImageLabel(QLabel):
    ...
    @classmethod
    def prefetch(cls, urls: Iterable[str], *, resolution: int=_PREFETCH_RESOLUTION, max_enqueue: int=20) -> None:
        try:
    ...
    @classmethod
    def prefetch_deferred(cls, urls: Iterable[str], *, resolution: int=_PREFETCH_RESOLUTION, chunk: int=16, delay_ms: int=40) -> None:
        """Spread prefetch work across event-loop ticks to avoid UI stalls."""
        normalized: list[str] = []
        seen: set[str] = set()
        for raw in urls:
            url = cls._normalize_url(raw, resolution=resolution)
        ...
        def _enqueue(start: int = 0) -> None:
            if start >= len(normalized):
                return
            cls.prefetch(normalized[start:start + chunk], resolution=resolution, max_enqueue=chunk)
            if start + chunk < len(normalized):
                QTimer.singleShot(delay_ms, lambda s=start + chunk: _enqueue(s))
    ...
    def _pump_queue(cls) -> None:
        try:
            ...
            cap = 3 if in_startup_window() else cls._MAX_CONCURRENT
            while cls._queued_high or cls._queued_low:
                if len(cls._active_urls) >= cap:
                    return
    ...
    def _apply_pixmap(self, pixmap: QPixmap) -> None:
        mode = Qt.TransformationMode.FastTransformation if max(self.width(), self.height()) <= 80 else Qt.TransformationMode.SmoothTransformation
        scaled = pixmap.scaled(self.size(), Qt.AspectRatioMode.KeepAspectRatioByExpanding, mode)
